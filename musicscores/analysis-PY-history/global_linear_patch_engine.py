...
def find_word_end_note_linear(elements, start_idx):
    ...
    Verified flawless during the localized test run.
    """
...
def run_global_linear_repair_engine():
    print("=" * 115)
    # Register namespaces to prevent 'ns0:' tags from corrupting the schema wrapper globally
    ET.register_namespace('', 'http://idpf.org') 
    print("MUSICXML GLOBAL LINEAR PATCH ENGINE: EXECUTING STREAM-BASED WORD-LOCKED REPAIRS")
    print("=" * 115)
    ...
    # Pre-gather all target XML files across the whole drive structure
    all_xml_files = []
    ...
    for full_file_path in all_xml_files:
        try:
            ...
            # Step 1: Flatten everything inside all measures sequentially into a continuous timeline
            linear_flow = []
                    ...
                    if ZAQEF_QATON_GLYPH in dir_xml:
                        ...
                        if target_note is not None:
                            ...
                            if not has_breath:
                                ...
                                # Schema-compliant sequence placement (Insert right before lyric/existing notation tags)
            ...
            # Step 3: Backup pristine copy and overwrite with precision updates
            if file_changed:
            ...
            # Gracefully bypass locked files or schema read errors
            pass

    print("\n" + "=" * 115)
    print("PATCH OPERATION FINALIZED SUCCESSFULLY.")
