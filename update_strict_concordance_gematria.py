...
def get_color_class(val):
    if val < 50: return "gem-low"
    elif val < 200: return "gem-mid-low"
    elif val < 400: return "gem-mid"
    elif val < 600: return "gem-mid-high"
    else: return "gem-high"
...
def inject_gematria_to_files(directory_path):
    ...
    for filename in os.listdir(directory_path):
        if filename.endswith(".html"):
            ...
            # 2. Fix ALL Data Lines within the grid blocks
            data_blocks = soup.find_all('div', class_='level2')
            for block in data_blocks:
                # Find ALL Hebrew words and ALL reference cells in this block
                hebrew_cells = block.find_all('div', class_='tdcl6')
                ref_cells = block.find_all('div', class_='tdcl9')
                
                # Iterate through pairs to keep every multi-line row uniform
                for i, ref_cell in enumerate(ref_cells):
                    # Check if a badge already exists right next to this specific reference cell
                    next_sibling = ref_cell.find_next_sibling('div')
                    if next_sibling and 'tdcl_gem' in next_sibling.get('class', []):
                        continue # Skip if already processed
                    
                    # Match with corresponding Hebrew word for this row line
                    if i < len(hebrew_cells):
                        ...
                        if gematria_val > 0:
                            ...
                            # Insert immediately after this specific reference row cell
            ...
            if modified:
                with open(file_path, 'w', encoding='utf-8') as f:
                    f.write(str(soup))
                print(f"Fixed layout grid uniformity for: {filename}")
