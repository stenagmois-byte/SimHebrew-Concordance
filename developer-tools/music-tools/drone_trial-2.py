...
DRONE_NOTE_MAP = {"c":"48", "d":"50", "e":"52", "f":"53", "g":"55", "A":"57", "B":"59", "C":"60"}

import xml.etree.ElementTree as ET

def inject_native_musescore4_drone(root_xml, target_pitch):
    if target_pitch not in DRONE_NOTE_MAP:
        return False
    ...
    if score_node.find(".//Part[@id='2']") is None:
        ...
        # Locate Part 1 to find the insertion point
        part1 = score_node.find("./Part[@id='1']")
        if part1 is not None:
            # Find index of Part 1 in children list and insert Part 2 directly after it
            children = list(score_node)
            part1_index = children.index(part1)
            score_node.insert(part1_index + 1, cello_element)
        else:
            # Fallback if Part 1 lacks id="1" attribute explicitly
    ...
    for orig_meas in staff1.findall("Measure"):
        dr_meas = ET.fromstring(ET.tostring(orig_meas))
        
        # Clear out original voice components
        for voice in dr_meas.findall("voice"):
            dr_meas.remove(voice)
        ...
        if total_quarters <= 1:
            ...
            ET.SubElement(dr_note, "pitch").text = str(midi_val)
            ET.SubElement(dr_note, "tpc").text = "14"
