...
from PIL import Image  # Handles the low-colour 8-bit quantization
...
def flatten_svg_to_optimized_png(temp_dir):
    """Converts SVGs to high-res, then scales down color channels to 8-bit palette format."""
    svg_mapping = {}
    ...
    for svg_path in temp_dir.rglob("*.svg"):
        ...
        try:
            ...
            # Write a temporary 24-bit TrueColor image via ReportLab
            renderPM.drawToFile(drawing, str(png_path), fmt="PNG")
            
            # 2. Low-Colour Palette Post-Processing Optimization
            with Image.open(png_path) as img:
                # Convert image to grayscale (L) to cleanly merge anti-aliased edge colors
                grayscale_img = img.convert("L")
                # Quantize down to a tight 8-color indexed palette (adaptive, zero-dither)
                low_color_img = grayscale_img.quantize(colors=8, dither=Image.Dither.NONE)
                # Overwrite original 24-bit file with our sleek, compact 8-bit copy
...
def modify_css_classes(temp_dir):
...
def process_volume(epub_path):
    ...
    modify_css_classes(temp_extract_dir)
...
def main():
    ...
    if not epub_files:
        ...
        return
    print(f"Found {len(epub_files)} volumes. Executing low-colour batch production run...")
    for epub_file in epub_files:
        process_volume(epub_file)
    print("\nAll optimized conversions finished successfully!")
