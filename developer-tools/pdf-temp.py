...
def strip_epub_structural_cover(original_epub_path, temp_epub_path):
    ...
    try:
        with zipfile.ZipFile(original_epub_path, 'r') as src, zipfile.ZipFile(temp_epub_path, 'w', zipfile.ZIP_DEFLATED) as dst:
            ...
            # 1. Locate the internal structural index file (.opf)
            opf_file_name = None
            ...
            # 2. Read and parse the structural package XML
            opf_content = src.read(opf_file_name)
            namespaces = {'opf': 'http://idpf.org'}
            # Register namespaces to maintain tag naming structure during save
            ElementTree.register_namespace('', 'http://idpf.org')
            root = ElementTree.fromstring(opf_content)
            
            cover_item_ids = set()
            
            # --- Neutralize Step A: Remove cover metadata tags ---
            metadata = root.find(".//opf:metadata", namespaces)
            if metadata is not None:
                # Find meta names pointing to cover image IDs
                for meta in metadata.findall(".//opf:meta[@name='cover']", namespaces):
                    cover_item_ids.add(meta.get('content'))
                    metadata.remove(meta)
                # Remove Calibre specific cover properties if present
                for meta in metadata.findall(".//opf:meta", namespaces):
                    if meta.get('property') == 'calibre:title_sort':
                        pass # Keep title sorting intact
            
            # --- Neutralize Step B: Unmark items in the manifest ---
            ...
            if manifest is not None:
                for item in manifest.findall(".//opf:item", namespaces):
                    ...
                    # If item is explicitly labeled a cover or matches the cover ID
            ...
            if guide is not None:
                ...
                # If the guide section is completely empty now, remove it entirely
                if len(guide) == 0:
                    root.remove(guide)

            # 3. Convert the modified XML tree back to a string
            updated_opf_content = ElementTree.tostring(root, encoding='utf-8', xml_declaration=True)

            # 4. Copy everything into the temporary EPUB package
            for item in src.infolist():
                if item.filename == opf_file_name:
                    # Write our new structural code instead of the original index
                    dst.writestr(item, updated_opf_content)
                else:
                    filename_lower = os.path.basename(item.filename).lower()
                    # Skip writing physical titlepage or cover files if they are loose html pages
...
def generate_custom_paperback_pdfs():
    print("=" * 85)
    print("SAFE SEQUENTIAL PAPERBACK PDF PRODUCTION GENERATOR - STRUCTURAL COVER STRIP")
    print("=" * 85)
    ...
    for idx, epub_path in enumerate(epub_files, start=1):
        ...
        # Modify the structural configuration records on the fly
