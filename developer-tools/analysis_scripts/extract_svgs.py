...
def get_active_book_folder():
    ...
    epub_stem = epubs[0].stem  # e.g., "daniel_ezra_nehemiah"
    
    if " - " in epub_stem:
        base_name = epub_stem.split(" - ")[0].strip().lower()
    else:
        base_name = epub_stem.strip().lower()
        
    book_tokens = [token for token in re.split(r'[_ \-]+', base_name) if token]
    ...
    if MUSIC_SCORES_BASE.exists():
        for item in MUSIC_SCORES_BASE.iterdir():
            ...
            if base_name in folder_name_lower or any(token in folder_name_lower for token in book_tokens):
                print(f"🎯 Project Detected: '{epub_stem}' mapped to directory: '{item.name}'")
                return item.name
                
    print(f"❌ ERROR: Could not find a GitHub folder matching tokens: {book_tokens} inside {MUSIC_SCORES_BASE}")
    return None
...
def main():
    ...
    # --- FIXED STRUCTURAL BLOCK ---
    all_scores = []  # Explicitly defined at the base level of the main function scope
